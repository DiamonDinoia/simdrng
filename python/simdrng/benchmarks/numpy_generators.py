"""Benchmark suite for simdrng generators vs NumPy builtins.
...
def bench_throughput(size: int = 100_000_000, repeat: int = 10) -> None:
    ...
    generators: dict[str, Callable[[], np.random.Generator]] = {
        "XoshiroSIMD": lambda: simdrng.XoshiroSIMD(1234),
        "XoshiroNative": lambda: simdrng.XoshiroNative(1234),
        "Xoshiro": lambda: simdrng.Xoshiro(1234),
        "SplitMix": lambda: simdrng.SplitMix(1234),
        "ChaCha20": lambda: simdrng.ChaCha20(1234),
        "ChaCha8": lambda: simdrng.ChaCha8(1234),
...
def bench_bulk_vs_generator(size: int = 10_000_000, repeat: int = 10) -> None:
    _header(f"3. simdrng.random() vs NumPy.random() (N={size:_})")
    out = np.empty(size, dtype=np.float64)

    # simdrng (transparent C++ bulk fill via Generator subclass)
    for name, rng in [
        ("XoshiroSIMD", simdrng.XoshiroSIMD(1234)),
        ("XoshiroNative", simdrng.XoshiroNative(1234)),
    ]:
        def run(r=rng, o=out):
            r.random(size, out=o)
...
def bench_serialization(repeat: int = 10_000) -> None:
    ...
    generators = {
        "XoshiroSIMD": simdrng.XoshiroSIMD(1234),
        "ChaCha20": simdrng.ChaCha20(1234),
        "SplitMix": simdrng.SplitMix(1234),
    }
...
def bench_integers(size: int = 10_000_000, repeat: int = 10) -> None:
    ...
    generators = {
        "XoshiroSIMD bulk": simdrng.XoshiroSIMD(1234),
        "XoshiroNative bulk": simdrng.XoshiroNative(1234),
        "ChaCha20 bulk": simdrng.ChaCha20(1234),
    }
    for name, rng in generators.items():
        def run(r=rng, o=out):
