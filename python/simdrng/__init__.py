...
try:
    from simdrng_ext import _XoshiroNative
except ImportError:
    _XoshiroNative = None

try:
    from simdrng_ext import _ChaCha8Native, _ChaCha12Native, _ChaCha20Native
except ImportError:
...
class BitGeneratorBase:
    """Base class for all simdrng bit generators.
...
def seed(s: int = 42, *, generator: str = "XoshiroSIMD") -> None:
    ...
    After calling ``simdrng.seed(42)``, the module-level convenience
    functions (``simdrng.random``, ``simdrng.normal``, etc.) use this
    generator.
